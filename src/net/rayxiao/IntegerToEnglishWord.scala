...
object IntegerToEnglishWord extends App{
  ...
  def processInt(remainingInt:Long):String = remainingInt match {
    case x if x>=1000000000 => processInt(x/1000000000) +" Billion " + processInt(x%1000000000)
    case x if x>=1000000 => processInt(x/1000000) + " Million " +processInt(x%1000000)
    case x if x>=1000 => processInt(x/1000) + " Thousands " + processInt(x%1000)
    case x if x>=100 =>processInt(x/100) + " Hundreds " + processInt(x%100)
